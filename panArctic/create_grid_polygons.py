...
def create_grids_for_overlap_vectors(coverage,input_vector,grid_size_x,grid_size_y,save_path, exclude_id_txt=None):
    ...
    # for change to shapefile if needed
    save_path = save_path.replace('.gpkg','.shp')

    # "grid_id" is the same as "RowCol_id",  for compatibility with existing workflow
    intersecting_gdf[['geometry', 'RowCol_id','grid_id']].to_file(save_path, driver='ESRI Shapefile')
