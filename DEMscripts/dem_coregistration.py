...
def statistic_demcoreg_mode_performance(coreg_json_list, save_path):
    # coreg_json_list: a list of json files storing the metadata of co-registration results, which are generated by get_meta_of_coreg_dem function and saved in the same folder as the co-registration results.
    # save_path: the path to save the performance statistics, in csv format.

    # read all json files and extract the metadata into a dataframe
    meta_list = []
    for json_file in coreg_json_list:
        meta_dict = io_function.read_dict_from_txt_json(json_file)
        # convert sub-level dict in meta_dict to str, for example, the "shift" dict to "shift_dx", "shift_dy", "shift_dz", "shift_dm"
        new_meta_dict = {}
        for key, value in meta_dict.items():
            if isinstance(value, dict):
                for subkey, subvalue in value.items():
                    new_meta_dict[f"{key}_{subkey}"] = subvalue
            else:
                new_meta_dict[key] = value
        meta_list.append(new_meta_dict)
    meta_df = pd.DataFrame(meta_list)
    # print(meta_df.columns)
    # print(meta_df)
    # return 0

    # "start_time": "2026-03-04 13:10:52", convert start_time and end_time to datetime format
    meta_df['start_time'] = pd.to_datetime(meta_df['start_time'])
    meta_df['end_time'] = pd.to_datetime(meta_df['end_time'])

    # calculate some performance statistics, for example, mean and median of shift before and after co-registration, mean and median of elevation difference before and after co-registration, etc.
    performance_stats = {'min': {}, 'max': {}, 'mean': {}, 'median': {}}
    for attr in ['mode','max_dz','shift_dx','shift_dy','shift_dz','shift_dm','max_offset','Iteration_count','processing_time_sec','start_time','end_time']:
        if attr in ['mode']:
            performance_stats['min'][f'{attr}'] = meta_df[attr].unique().tolist()[0]
            performance_stats['max'][f'{attr}'] = meta_df[attr].unique().tolist()[0]
            performance_stats['mean'][f'{attr}'] = meta_df[attr].unique().tolist()[0]
            performance_stats['median'][f'{attr}'] = meta_df[attr].unique().tolist()[0]
        else:
            performance_stats['min'][f'{attr}'] = meta_df[attr].min()
            performance_stats['max'][f'{attr}'] = meta_df[attr].max()
            performance_stats['mean'][f'{attr}'] = meta_df[attr].mean()
            performance_stats['median'][f'{attr}'] = meta_df[attr].median()

    # performance_stats['earliest_start_time'] = meta_df['start_time'].min() 
    # performance_stats['latest_end_time'] = meta_df['end_time'].max() 
    for stat in ['min', 'max', 'mean', 'median']:
        performance_stats[stat]['total_time_sec'] = (performance_stats['max'][f'{attr}'] - performance_stats['min'][f'{attr}']).total_seconds()

    # print(performance_stats)

    # save the performance statistics to xlsx
    performance_stats_df = pd.DataFrame(performance_stats)
    # switch the column and row
    performance_stats_df = performance_stats_df.T
    # add first column as "statistic"
    performance_stats_df.insert(0, 'statistic', performance_stats_df.index)

    performance_stats_df.to_excel(save_path, index=False)
    print(f'Saved performance statistics to {save_path}')


def test_statistic_demcoreg_mode_performance():
    data_dir = os.path.expanduser('~/Data/dem_processing/registration_tifs')
    # data_dir = os.path.expanduser('~/data3/pDEMtools_tmp_dir/registration_tifs')

    res_dir_list = io_function.get_file_list_by_pattern(data_dir,"dem_grid0016000342_coreg_????")
    if len(res_dir_list) == 0:
        print(f'No co-registration results found in {data_dir}, please check the path and the pattern')
        return
    for res_dir in res_dir_list:

        # res_dir = os.path.join(data_dir,'dem_grid0016000342_coreg_nuth')
        print(f'\nProcessing results in {res_dir}')

        coreg_json_list = io_function.get_file_list_by_pattern(res_dir, "*_meta.json")
        print(f'Found {len(coreg_json_list)} co-registration metadata json files')

        save_path = os.path.join(res_dir, f'{os.path.basename(res_dir)}_coreg_performance_stats.xlsx')
...
if __name__ == '__main__':
    ...
    # test_statistic_demcoreg_mode_performance()
    # sys.exit(0)
